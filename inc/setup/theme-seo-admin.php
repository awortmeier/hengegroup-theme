...
//   - a "SEO" meta box on every post type in base_theme_get_seo_post_types() (default: post,
//     page; filterable via `base_theme_seo_post_types` for project themes with custom post
...
// see base_theme_render_seo_image_picker_field()/base_theme_enqueue_seo_media_picker() below --
...
const BASE_THEME_SEO_OPTION = 'base_theme_seo_options';
 ...
 */
function base_theme_get_seo_options(): array
{
    $defaults = [
 ...
 */
function base_theme_get_seo_post_types(): array
{
    $post_types = apply_filters('base_theme_seo_post_types', ['post', 'page']);

    return is_array($post_types) ? $post_types : ['post', 'page'];
}

function base_theme_sanitize_seo_options($input): array
{
    $input = is_array($input) ? $input : [];
    $defaults = base_theme_get_seo_options();
...
function base_theme_action_admin_init_register_seo_settings(): void
{
    register_setting('base_theme_seo_options_group', BASE_THEME_SEO_OPTION, [
        'type' => 'array',
        'sanitize_callback' => 'base_theme_sanitize_seo_options',
        'default' => [],
    ]);
}
add_action('admin_init', 'base_theme_action_admin_init_register_seo_settings');

function base_theme_action_admin_menu_seo_settings(): void
{
    add_options_page(
        __('SEO', 'base-theme'),
        __('SEO', 'base-theme'),
        'manage_options',
        'base-theme-seo',
        'base_theme_render_seo_settings_page',
    );
}
add_action('admin_menu', 'base_theme_action_admin_menu_seo_settings');
 ...
 * `.base-theme-seo-image-picker` on the page generically, so this markup is the only thing either
 * caller needs to render.
 */
function base_theme_render_seo_image_picker_field(
    string $field_id,
    string $field_name,
    int $attachment_id,
): void {
    $image_url = $attachment_id > 0 ? wp_get_attachment_image_url($attachment_id, 'medium') : ''; ?>
    <div class="base-theme-seo-image-picker">
        <input
            type="hidden"
        ...
        />
        <div class="base-theme-seo-image-picker__preview" style="margin-bottom: 8px">
        ...
        <button type="button" class="button base-theme-seo-image-picker__select">
            <?php esc_html_e('Bild auswaehlen', 'base-theme'); ?>
        </button>
        <button
            type="button"
            class="button base-theme-seo-image-picker__remove"
            <?php echo $attachment_id > 0 ? '' : 'style="display: none"'; ?>
        >
            <?php esc_html_e('Entfernen', 'base-theme'); ?>
        </button>
    </div>
    <?php
}

function base_theme_enqueue_seo_media_picker(): void
{
...
function base_theme_action_admin_enqueue_scripts_seo(string $hook_suffix): void
{
    if ($hook_suffix === 'settings_page_base-theme-seo') {
        base_theme_enqueue_seo_media_picker();
        return;
    }
    ...
    if (!$screen || !in_array($screen->post_type, base_theme_get_seo_post_types(), true)) {
        return;
    }

    base_theme_enqueue_seo_media_picker();
}
add_action('admin_enqueue_scripts', 'base_theme_action_admin_enqueue_scripts_seo');
 ...
 */
function base_theme_render_seo_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('SEO', 'base-theme'); ?></h1>
        <p>
            <?php esc_html_e(
        ...
        <form action="options.php" method="post">
            <?php settings_fields('base_theme_seo_options_group'); ?>
            <table class="form-table" role="presentation">
                <tbody>
                    <tr>
                        <th scope="row">
                            <label for="base_theme_seo_options_title_template">
                                <?php esc_html_e('Titel-Vorlage', 'base-theme'); ?>
                            </label>
                        </th>
                        <td>
                            <input
                                type="text"
                                id="base_theme_seo_options_title_template"
                    ...
                    <tr>
                        <th scope="row">
                            <label for="base_theme_seo_options_title_separator">
                                <?php esc_html_e('Titel-Trenner', 'base-theme'); ?>
                            </label>
                        </th>
                        <td>
                            <input
                                type="text"
                                id="base_theme_seo_options_title_separator"
                                name="<?php echo esc_attr(
                                    BASE_THEME_SEO_OPTION,
                                ); ?>[title_separator]"
                    ...
                    <tr>
                        <th scope="row">
                            <label for="base_theme_seo_options_description">
                                <?php esc_html_e('Standard Meta-Beschreibung', 'base-theme'); ?>
                            </label>
                        </th>
                        <td>
                            <textarea
                                id="base_theme_seo_options_description"
                    ...
                    <tr>
                        ...
                        <td>
                            <?php base_theme_render_seo_image_picker_field(
                                'base_theme_seo_options_og_image',
                    ...
                    <tr>
                        <th scope="row">
                            <label for="base_theme_seo_options_twitter_handle">
                                <?php esc_html_e('Twitter/X-Handle', 'base-theme'); ?>
                            </label>
                        </th>
                        <td>
                            <input
                                type="text"
                                id="base_theme_seo_options_twitter_handle"
                                name="<?php echo esc_attr(
                                    BASE_THEME_SEO_OPTION,
                                ); ?>[twitter_handle]"
...
function base_theme_action_add_meta_boxes_seo(): void
{
    foreach (base_theme_get_seo_post_types() as $post_type) {
        add_meta_box(
            'base_theme_seo',
            __('SEO', 'base-theme'),
            'base_theme_render_seo_meta_box',
            $post_type,
            'normal',
            'high',
        );
...
add_action('add_meta_boxes', 'base_theme_action_add_meta_boxes_seo');
 ...
 */
function base_theme_render_seo_meta_box(WP_Post $post): void
{
    wp_nonce_field('base_theme_seo_save_' . $post->ID, 'base_theme_seo_nonce');

    $title = (string) get_post_meta($post->ID, '_base_theme_seo_title', true);
    $description = (string) get_post_meta($post->ID, '_base_theme_seo_description', true);
    $image_id = (int) get_post_meta($post->ID, '_base_theme_seo_image_id', true);
    $canonical = (string) get_post_meta($post->ID, '_base_theme_seo_canonical', true);
    $robots_index = (string) get_post_meta($post->ID, '_base_theme_seo_robots_index', true);
    $robots_follow = (string) get_post_meta($post->ID, '_base_theme_seo_robots_follow', true);

    $options = base_theme_get_seo_options();
    ?>
    <p>
        <label for="base_theme_seo_title">
            <strong><?php esc_html_e('SEO-Titel', 'base-theme'); ?></strong>
        </label>
        <br />
        <input
            type="text"
            id="base_theme_seo_title"
            name="base_theme_seo[title]"
        ...
        <span class="description">
            <?php echo esc_html(
                sprintf(
                    ...
                    __('Leer = automatisch aus Titel-Vorlage ("%s").', 'base-theme'),
    ...
    <p>
        <label for="base_theme_seo_description">
            <strong><?php esc_html_e('Meta-Beschreibung', 'base-theme'); ?></strong>
        </label>
        <br />
        <textarea
            id="base_theme_seo_description"
            name="base_theme_seo[description]"
            rows="3"
            class="large-text"
    ...
    <p>
        <label for="base_theme_seo_canonical">
            <strong><?php esc_html_e('Kanonische URL', 'base-theme'); ?></strong>
        </label>
        <br />
        <input
            type="url"
            id="base_theme_seo_canonical"
            name="base_theme_seo[canonical]"
    ...
    <p>
        <strong><?php esc_html_e('Social-Bild', 'base-theme'); ?></strong>
        <br />
        <?php base_theme_render_seo_image_picker_field(
            'base_theme_seo_image_id',
            'base_theme_seo[image_id]',
            $image_id,
        ); ?>
        <span class="description">
            <?php esc_html_e(
    ...
    <p>
        <label for="base_theme_seo_robots_index">
            <strong><?php esc_html_e('Indexierung', 'base-theme'); ?></strong>
        </label>
        <br />
        <select id="base_theme_seo_robots_index" name="base_theme_seo[robots_index]">
            <option value="" <?php selected($robots_index, ''); ?>>
                <?php esc_html_e('Standard verwenden', 'base-theme'); ?>
            </option>
            <option value="index" <?php selected($robots_index, 'index'); ?>>
                <?php esc_html_e('Indexieren', 'base-theme'); ?>
            </option>
            <option value="noindex" <?php selected($robots_index, 'noindex'); ?>>
                <?php esc_html_e('Nicht indexieren (noindex)', 'base-theme'); ?>
            </option>
        </select>
    </p>
    <p>
        <label for="base_theme_seo_robots_follow">
            <strong><?php esc_html_e('Links folgen', 'base-theme'); ?></strong>
        </label>
        <br />
        <select id="base_theme_seo_robots_follow" name="base_theme_seo[robots_follow]">
            <option value="" <?php selected($robots_follow, ''); ?>>
                <?php esc_html_e('Standard verwenden', 'base-theme'); ?>
            </option>
            <option value="follow" <?php selected($robots_follow, 'follow'); ?>>
                <?php esc_html_e('Folgen', 'base-theme'); ?>
            </option>
            <option value="nofollow" <?php selected($robots_follow, 'nofollow'); ?>>
                <?php esc_html_e('Nicht folgen (nofollow)', 'base-theme'); ?>
            </option>
        </select>
    </p>
    <?php
...
function base_theme_update_or_delete_post_meta(int $post_id, string $meta_key, string $value): void
{
    if ($value === '') {
...
function base_theme_action_save_post_seo(int $post_id): void
{
    if (!isset($_POST['base_theme_seo_nonce'])) {
        return;
    }

    $nonce = sanitize_text_field(wp_unslash($_POST['base_theme_seo_nonce']));
    if (!wp_verify_nonce($nonce, 'base_theme_seo_save_' . $post_id)) {
        return;
    }
    ...
    if (!in_array(get_post_type($post_id), base_theme_get_seo_post_types(), true)) {
        return;
    }
    ...
    $data =
        isset($_POST['base_theme_seo']) && is_array($_POST['base_theme_seo'])
            ? wp_unslash($_POST['base_theme_seo']) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
            : [];

    base_theme_update_or_delete_post_meta(
        $post_id,
        '_base_theme_seo_title',
        sanitize_text_field((string) ($data['title'] ?? '')),
    );
    base_theme_update_or_delete_post_meta(
        $post_id,
        '_base_theme_seo_description',
        sanitize_textarea_field((string) ($data['description'] ?? '')),
    );
    base_theme_update_or_delete_post_meta(
        $post_id,
        '_base_theme_seo_canonical',
        esc_url_raw((string) ($data['canonical'] ?? '')),
    );

    $image_id = absint($data['image_id'] ?? 0);
    if ($image_id > 0) {
        update_post_meta($post_id, '_base_theme_seo_image_id', $image_id);
    } else {
        delete_post_meta($post_id, '_base_theme_seo_image_id');
    }

    $robots_index = (string) ($data['robots_index'] ?? '');
    base_theme_update_or_delete_post_meta(
        $post_id,
        '_base_theme_seo_robots_index',
    ...
    base_theme_update_or_delete_post_meta(
        $post_id,
        '_base_theme_seo_robots_follow',
        in_array($robots_follow, ['follow', 'nofollow'], true) ? $robots_follow : '',
    );
}
add_action('save_post', 'base_theme_action_save_post_seo');
