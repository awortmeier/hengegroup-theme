...
while ($nextIndex -lt $totalFiles -or $activeJobs.Count -gt 0) {
    while (-not $stopQueueing -and $nextIndex -lt $totalFiles -and $activeJobs.Count -lt $maxParallelUploads) {
        ...
        $remoteUrl = "${ftpScheme}://${ftpHost}/${remoteBasePath}/${relativePath}"

        Write-Progress `
            -Activity "Uploading theme via FTP" `
            -Status "Starting upload $($nextIndex + 1) of ${totalFiles}: ${relativePath}" `
...
Write-Progress -Activity "Uploading theme via FTP" -Completed

if ($failedUploads.Count -gt 0) {
    throw "FTP upload failed for $($failedUploads -join ', ')"
}

Write-Host "Uploaded $totalFiles file(s) from dist to $ftpHost/$remoteBasePath."
